...
$error = false;
$fname = $lname = $email = $date_of_birth = $pass = $picture = $phone = '';
$fnameError = $lnameError = $emailError = $dateError = $passError = $picError = $phoneError = '';
if (isset($_POST['btn-signup'])) {
  ...
  if (!$error) {
      $query = "INSERT INTO `users`(`email`, `phone`, `first_name`, `last_name`, `password`, `date_of_birth`, `picture`) VALUES
       ('$email','$phone','$fname','$lname','$password','$date_of_birth','$picture->fileName')";
...
<body>
  <div class="container">
      <form class="w-75" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" autocomplete="off" enctype="multipart/form-data">
          ...
          <input type="text" name="phone" class="form-control" placeholder="Enter your phone number" maxlength="25" />
